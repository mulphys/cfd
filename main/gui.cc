...
)
{
	...
	{	int	loc=var->loc;
		...
		{	case nodes:
			...
			///	break;
			default:
			fprintf(stderr,"Can't display scalar variable type %d\n",type);
		}
	}
	else
	{
...
void	displayAxes()
{
	...
	glutSolidCone ((GLdouble)(axes[arrowidth]*lz), (GLdouble)(axes[arrowheight]*lz), (GLint)4, (GLint)4);
	glTranslatef (-xo[0], -xo[1], -(xo[2]+axes[zaxislength]*lz));
//?	glFlush();        
}
...
{	float	color[3];
	...
	{	int icolorvar=(int)P.disp[variable];
		struct Point	*p,*origin;
		if (dom->variable[icolorvar].size == 0)
			showpar[showSpheres] = 0;
...
///		{"pvel",showPartVelLimits},
		{(char*)"bv",toggleBoundaryVectors},
		{(char*)"dom",selectCurrentDomain},
		{(char*)"v",selectVariable},
///#endif
		{(char*)"m",consoleMenu},
		{(char*)"w",writeData},
		{(char*)"wf",setWriteFormat},
		{(char*)"lim",printGridXLimits},
		{(char*)"gvec",printGridVecLimits},
...
//	win = drawable;//?
//	glXMakeCurrent(dpy, drawable, cx);//?
...
//-
//-	glxwin = glXCreateWindow (
//-		dpy, fbConfigs[0], win, NULL
//-	);
//-
...
#endif
//	glutSetWindow(win); //?
//https://www.opengl.org/discussion_boards/showthread.php/124363-Find-XWindow-id-for-GLUT-based-program
	myinit(argc, argv);
	helpDisplay();
