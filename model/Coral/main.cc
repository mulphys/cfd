...
#define	MAXPRESITER	8	//number of pressure sub-iterations
	...
	(//Asymmetric parabolic branching
		int	level,
		double	size,
		double	r,  //position on the parabolic branch
		double	*x0,
		double	*e0,
		Tool	*tool
	);
...
///(	Point	*a,
///	double *xa,//Coordinates of a point 
///	double *X, //Global coordinates array
///	double *M, //Global mass-array
///	double *fframe //force of the frame
///)
...
///			double	d[DIM],dd,//distance between 'a' and 'b'
///				*xb=X+jmb,
...
{	//Create a binary tree
	...
	double	x[DIM],e[DIM];//initial position and direction
...
void	stepModCoral(double dt, Domain *dom)
...
//	while(tool_boundary_nodes>0)
		if (tool_boundary_nodes>0)
			tool_boundary_nodes=tool->growCoral(dom);
...
{	int
	...
	{	randvec(e);//random unit vector
		...
		r2=0.0;
		for (int i=0; i<DIM; i++)
		{	double	a=(1.-branching_factor)*e0[i]+branching_factor*e[i];
			r2+=a*a;
			e[i]=a;
		}
		r=sqrt(r2);
	...
	{	r=stem_radius+(double)(i+1)/(double)ntail*(next_radius-stem_radius);
		for (int k=0; k<DIM; k++)
			x[k]+=segment_length*e0[k];
		//create particle 
...
//next_segment_length0=next_segment_length1;//=segment_length;///DDD
	...
	//Select new direction
	r=1./LENGTH(e0);
	for(int i=0;i<DIM;i++)e1[i]=e0[i]*r;
	do
	{	randvec(e);//random unit vector
		//Determine the new direction
///			r2=0.0;
...
///			branching=sqrt(branching);
		branching=SCLP(e1,e);
//printf("e=%g,%g,%g,branching=%g\n",e[0],e[1],e[2],branching);
	} while(branching<0.81||branching>0.84);//limit the branching angle
	tail_length=r1*next_radius1/branching;
	ntail=(int)ceil(tail_length/next_segment_length1)+1;
	//Do ntail steps 
	segment_length=tail_length/ntail;
	for (int i=0; i<ntail; i++)
	{	r=stem_radius+(double)(i+1)/(double)ntail*(next_radius1-stem_radius);
		for (int k=0; k<DIM; k++)
			x[k]+=segment_length*e0[k];
		//create particle 
		tool->addnode(x,(int)boundary,r,refinement_factor);
...
(	int	level,
	double	stem_radius,
	double	r,  //position on the parabolic branch
	double	*x0,//start coordinate
	double	*e0, //start direction of the small branch
	Tool	*tool
)
{	int
		nstem,//number of points along the branch stem
...
//-	for (int i=0; i<ntail; i++)
//-	{	int	ip;
//-		double	size=stem_radius+(double)(i+1)/(double)ntail*(next_radius-stem_radius);
//-		for (int k=0; k<DIM; k++)
//-			x[k]+=segment_length*e0[k];
//-		//create particle 
//-		if ((ip=putp(x))<0) continue;
